...
// Generated once, on-host, via WSL's libxkbcommon (see build-scripts/ for
// context — NOT regenerated on every build, and NOT built for Android at
// all: cross-compiling libxkbcommon just to generate a keymap wasn't worth
// it, since zwp_virtual_keyboard_v1 just needs the resulting XKB_V1 TEXT
// format keymap as a static blob, not the library itself.
//
// Fixed 'us'/pc105 layout. Not user-configurable yet — see host-bridge
// README for what that would take (a settings-driven keymap swap, still
// just static text, no on-device xkbcommon needed either way).

namespace hostbridge {

inline constexpr char kDefaultXkbKeymapUS[] = R"XKB_RAW_DELIM(
xkb_keymap {
