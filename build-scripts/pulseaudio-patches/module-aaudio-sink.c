/***
  ...
  droidtop: Termux's module-aaudio-sink.c as of its PulseAudio 13.0
  package (termux-packages 39437706663c, packages/pulseaudio), extended
  with the three arguments GameNative's PulseAudioComponent passes
  ("volume", "performance_mode", "low_latency"), which the arm64 module
  it ships accepts. See build-scripts/build-vendor-deps.sh.
...
PA_MODULE_USAGE(
    ...
    "volume=<output volume, linear, 1.0 is unity> "
    "performance_mode=<0 none, 1 low latency, 2 power saving> "
    "low_latency=<keep the buffer at two bursts: true/false> "
);
...
static int pa_open_aaudio_stream(struct userdata *u)
{
    ...
    if (u->low_latency) {
        int32_t burst = AAudioStream_getFramesPerBurst(u->stream);
        if (burst > 0)
            AAudioStream_setBufferSizeInFrames(u->stream, burst * 2);
    }
...
int pa__init(pa_module*m) {
    ...
    /* GameNative numbers performance modes 0 none, 1 low latency,
     * 2 power saving; "pm" above counts from AAUDIO_PERFORMANCE_MODE_NONE
     * (0 none, 1 power saving, 2 low latency). */
    if (pa_modargs_get_value(ma, "performance_mode", NULL)) {
        uint32_t mode = 1;
        pa_modargs_get_value_u32(ma, "performance_mode", &mode);
        u->pm = mode == 1 ? 2 : mode == 2 ? 1 : 0;
    }
    ...
    {
        const char *volume = pa_modargs_get_value(ma, "volume", NULL);
        double linear;
        if (volume && pa_atod(volume, &linear) >= 0 && linear >= 0) {
            pa_cvolume cv;
            pa_cvolume_set(&cv, u->ss.channels, pa_sw_volume_from_linear(linear));
            pa_sink_new_data_set_volume(&data, &cv);
        }
    }
