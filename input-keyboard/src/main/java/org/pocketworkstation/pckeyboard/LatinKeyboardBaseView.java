...
public class LatinKeyboardBaseView extends View implements PointerTracker.UIProxy {
    ...
    public LatinKeyboardBaseView(Context context, AttributeSet attrs, int defStyle) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // droidtop patch (not upstream Hacker's Keyboard): switch() on
            // R.styleable.* needs the case labels to be compile-time
            // constants, which library modules' R fields aren't under
            // android.nonTransitiveRClass=true (set project-wide) — real
            // compile error ("constant expression required"), not assumed.
            // Converted to if/else, same behavior otherwise.
